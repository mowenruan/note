...
\begin{enumerate}
  ...
  \item \textbf{控制残差流随深度累积}：若每个残差分支初始输出方差为常数，$L$ 个 block 累加后残差流尺度约随 $\sqrt{L}$ 增长。因此 GPT-2、GPT-3、Megatron/GPT-NeoX 一类模型会缩小残差分支输出投影。
\end{enumerate}
...
GPT-2 论文明确提出“modified initialization”：为了补偿残差路径随深度的累积，将残差层权重在初始化时乘以 $1/\sqrt{N}$，其中 $N$ 是残差层数量 \cite{gpt2paper}。工程实现中常把一个 Transformer block 的 attention output projection 和 MLP output projection 看作两个残差分支，因此写成
\[
  \sigma_{\mathrm{residual\ output}}=\frac{0.02}{\sqrt{2L}},
\]
其中 $L$ 是 block 数。这一思想后来被 GPT-3 继承 \cite{gpt3paper}，也影响了 Megatron-LM、GPT-NeoX 等训练框架。
...
\subsection{checkpoint 继承初始化}

指令微调、RLHF、DPO/RLAIF、长上下文继续训练等阶段通常不再从随机参数开始，而是从 base/pretrained checkpoint 初始化。此时最重要的是：

\begin{itemize}
  \item 新增参数（新 token、adapter、LoRA、classification head、reward head）如何初始化；
  \item 是否冻结原模型；
  \item 新旧词表、RoPE 频率、位置范围变化是否会造成尺度不匹配。
\end{itemize}

InstructGPT/ChatGPT 路线就是典型 checkpoint 继承：SFT 从 GPT-3 checkpoint 开始，reward model 和 PPO policy 也从已有语言模型或 SFT 模型初始化，而不是重新随机训练一个大模型 \cite{instructgpt}。LLaMA-2-Chat、Mistral-Instruct、Qwen-Instruct、DeepSeek-R1 等也都属于这一类。
...
\section{后训练与扩展阶段的初始化}

\subsection{指令微调与 RLHF}

大模型后训练通常从 base checkpoint 初始化。以 InstructGPT 为例，SFT policy 从 GPT-3 初始化，reward model 与 PPO policy 也从已有模型初始化 \cite{instructgpt}。这种初始化的优势非常明显：预训练语言能力和世界知识被保留，后训练只需改变行为分布、偏好和安全边界。

\textbf{风险}：如果学习率过大或 KL 约束过弱，checkpoint 初始化的优势会被 catastrophic forgetting 抵消；如果新增 reward/value head 初始化尺度过大，RL 初期优势估计会噪声很大。

\subsection{LoRA、adapter 与新 head}

参数高效微调通常冻结 base weights，只初始化少量新增参数。LoRA 的常见做法是让一个低秩矩阵随机初始化、另一个为零，使初始 adapter 对原模型函数近似无影响。这是一种“函数保持初始化”：训练开始时模型输出等于 base model，随后逐步学习增量。

\subsection{词表扩展}

扩展 tokenizer 时，旧 token embedding 从 checkpoint 继承，新 token embedding 常用以下策略：

\begin{itemize}
  \item 按原模型 embedding 的均值/方差采样；
  \item 用相关旧 token 的 embedding 平均值初始化；
  \item 随机 $\Normal(0,0.02^2)$ 或模型配置中的 `initializer\_range`。
\end{itemize}

若新 token 很多，随机初始化会导致早期 loss 波动，需要较长继续训练；若新 token 是特殊控制符，平均初始化或小方差初始化更稳。

\subsection{长上下文扩展}

RoPE 模型扩展上下文通常不新增大规模可学习参数，而是改变频率、插值或缩放策略，例如 NTK-aware scaling、YaRN、position interpolation 等。主体权重从短上下文 checkpoint 继承，再用长序列数据继续训练。此时“初始化”主要是位置函数和 optimizer state 的再启动问题。

\textbf{分析}：长上下文扩展的关键不是随机权重初始化，而是避免已学到的局部位置关系被破坏。位置缩放过猛会让中短上下文性能下降；继续训练数据如果长文本比例不足，也会导致模型只在形式上支持长上下文。
